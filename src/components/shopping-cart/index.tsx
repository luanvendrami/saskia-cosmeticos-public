...
export default function ShoppingCart() {
  ...
  // Efeito para lidar com cliques fora do carrinho
  useEffect(() => {
    if (!isCartOpen) {
    ...
    function handleOutsideClick(event: MouseEvent) {
      ...
      // Verifica se o clique foi em um botão de fechar modal
      const isCloseButton =
        target.closest('[aria-label="Close"]') ||
        target.closest('[aria-label="Fechar"]') ||
        target.closest(".MuiIconButton-root");

      // Se for um botão de fechar, não fecha o carrinho
      if (isCloseButton) {
        return;
      }

      // Verifica se está clicando dentro de algum modal
      ...
      // Se estiver clicando dentro de um modal, não fecha o carrinho
      if (isInConfirmationModal || isInDeliveryModal) {
        return;
      }

      // Se clicar fora do carrinho e não estiver em nenhum modal, fecha o carrinho
      if (
        cartRef.current &&
  ...
  }, [isCartOpen, toggleCart]); // Mantendo apenas as dependências necessárias
